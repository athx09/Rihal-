...
<body>
    ...
    <div class="container-xxl py-5">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="text-center wow fadeInUp" data-wow-delay="0.1s">
                        <h6 class="section-title bg-white text-center  px-3">Edit Destinations</h6>
      ...
      if(isset($_POST['edit'])){
        ...
        include('../connect.php');  
        $sql = $con->prepare("SELECT * FROM places WHERE ((name='$name' AND description='$desc') OR (name='$name' OR description='$desc')) AND place_id !='$place_id'");      
        $sql->execute();
        $rowsVVV = $sql->fetch();
        $count=$sql->rowCount();
        if($count <= 0){
            ...
            if($_FILES['image']['tmp_name'] == FALSE){
                ...
                $stmt = $con->prepare("UPDATE places SET  name = ? , description = ? , from_price = ? , to_price = ? , availability = ? , from_date = ? , to_date = ? , residence_type = ?  WHERE place_id = ?");
                $stmt->execute(array($name , $desc , $from_price , $to_price , $avail , $from_date , $to_date , $type , $place_id));
                ...
                echo '<div class="container" dir="ltr" style="margin-top:30px;color:#FFF;margin-bottom:30px;font-family:cairo">
                 ...
                 $stmt = $con->prepare("UPDATE places SET  name = ? , description = ? , from_price = ? , to_price = ? , availability = ? , from_date = ? , to_date = ? , residence_type = ? , image = ?  WHERE place_id = ?");
                $stmt->execute(array($name , $desc , $from_price , $to_price , $avail , $from_date , $to_date , $type , $image , $place_id));
                        ...
                        <div class="row g-3">
                            <div class="col-md-12">
                                <div class="form-floating">
                                    <input type="text" class="form-control" value="<?php echo $rowsSpec['name']; ?>" name="name" required id="name" placeholder="Name">
                                    <label for="name">Name</label>
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="form-floating">
                                    <textarea type="email" style="height: 300px" name="desc" required class="form-control" id="email" placeholder="Description"><?php echo $rowsSpec['description']; ?></textarea>
                                    <label for="email">Description</label>
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="form-floating">
                                    <input type="number" class="form-control" value="<?php echo $rowsSpec['from_price']; ?>" name="from_price" required id="subject" placeholder="Subject">
                                    <label for="subject">From Price</label>
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="form-floating">
                                    <input type="number" class="form-control" value="<?php echo $rowsSpec['to_price']; ?>" name="to_price" required id="subject" placeholder="Subject">
                                    <label for="subject">To Price</label>
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="form-floating">
                                    <input type="text" class="form-control" value="<?php echo $rowsSpec['availability']; ?>" name="avail" required id="subject" placeholder="Subject">
                                    <label for="subject">Availability</label>
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="form-floating">
                                    <input type="date" class="form-control" value="<?php echo $rowsSpec['from_date']; ?>" name="from_date" required id="subject" placeholder="Subject">
                                    <label for="subject">From Date</label>
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="form-floating">
                                    <input type="date" class="form-control" name="to_date" value="<?php echo $rowsSpec['to_date']; ?>" required id="subject" placeholder="Subject">
                                    <label for="subject">To Date</label>
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="form-floating">
                                    <input type="text" class="form-control" id="subject" value="<?php echo $rowsSpec['residence_type']; ?>" name="type" required placeholder="Subject">
                                    <label for="subject">Residence Type</label>
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="form-floating">
                                    <input type="file" class="form-control" id="subject" name="image" placeholder="Subject">
